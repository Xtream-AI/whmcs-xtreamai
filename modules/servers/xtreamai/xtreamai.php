...
(function () {
    function boot() {
        ...
        function apply() {
            ...
            if ($customer.length) { toggleCells($customer, !isReseller && !isTopUp); }
        }
...
function xtreamai_lineUsernameField(array $params): string
{
    ...
    foreach ($fields as $name => $value) {
        $key = xtreamai_configurableOptionKey((string) $name);
        if ($key !== 'line_username' && $key !== 'username' && $key !== 'panel_username') {
            continue;
        }
        if (is_array($value)) {
            $value = reset($value);
