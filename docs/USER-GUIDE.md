...
Yes. Set **Customer username** on the product to `Customer types it in the "Line username" custom field` and add a custom field named `Line username` (Text Box, ticked **Show on Order Form**) to that product: the customer types the username at checkout and the module uses it when it is valid and free on the panel. The module also checks the field in the cart, when the product is added and again on checkout, so a username that is taken is normally refused before the customer pays; when the order does get through with a taken username it stays pending with a clear message and you tell the customer to choose another one. The module never links a line that already exists from a customer order, so nobody can take over somebody else's line. See "Letting the customer choose the username" in section 6.
