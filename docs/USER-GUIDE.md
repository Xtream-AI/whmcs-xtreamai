...
| **Customer username** | Who chooses the username of the line. Only shown for `Line` products. | `Generated by the module (default)`: the module invents it, as always. `Customer types it in the "Line username" custom field`: the customer types it at checkout and the module uses it if it is valid and free. See "Letting the customer choose the username" in this section. |
...
By default the module invents the username of every line. If you want the customer to type it, open the product's **Module Settings** and set **Customer username** to `Customer types it in the "Line username" custom field`. This option only appears on **Line** products.
...
5. In **Validation** paste `^[A-Za-z0-9_-]{3,32}$`, so WHMCS rejects an invalid username in the cart before the order is placed.
6. Save.
...
The check happens when WHMCS creates the service, not when the customer orders, so a username can be free in the cart and taken a few minutes later. When that happens, edit the username on the service and press **Create** again, or ask the customer for another one. If you leave the custom field empty, or leave the option on `Generated by the module (default)`, nothing changes: usernames keep being generated as before.
...
Yes. Set **Customer username** on the product to `Customer types it in the "Line username" custom field` and add a custom field named `Line username` (Text Box, ticked **Show on Order Form**) to that product: the customer types the username at checkout and the module uses it when it is valid and free on the panel. If it is taken, the order stays pending with a clear message and you tell the customer to choose another one. The module never links a line that already exists from a customer order, so nobody can take over somebody else's line. See "Letting the customer choose the username" in section 6.
