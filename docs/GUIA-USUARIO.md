...
| **Customer username** | Quién elige el usuario. Se muestra en productos `Line` y `Sub-Reseller`, y se oculta en `Credit top-up`. | `Generated by the module (default)`: lo inventa el módulo, como siempre. `Customer types it in the "Line username" or "Reseller username" custom field`: lo escribe el cliente al hacer el pedido y el módulo lo usa si es válido y está libre. Mira "Dejar que el cliente elija su usuario" en esta sección. |
| **Customer password** | Quién elige la contraseña. Se muestra en productos `Line` y `Sub-Reseller`, y se oculta en `Credit top-up` (desde el módulo 1.10.0). | `Generated by the module (default)`: la crea el módulo con tus ajustes de contraseña. `Customer types it in the "Panel password" custom field`: la escribe el cliente al hacer el pedido y el módulo la usa si cumple las reglas. Mira "Dejar que el cliente elija su contraseña" en esta sección. |
...
| **Sub-Reseller Member Group ID** | Id numérico del grupo de miembros del panel al que pertenecerán las nuevas cuentas Sub-Reseller, sacado de la página **Member Groups** del panel. Es obligatorio cuando **Account Type** es `Sub-Reseller` y la clave del panel es de tipo **Admin**: sin él, Create se detiene con un mensaje claro y no crea nada. Las claves de Reseller lo ignoran y heredan el grupo desde su propia configuración de sub-reseller. | Por ejemplo `4`. |

### Dejar que el cliente elija su usuario

Por defecto el módulo inventa el usuario de cada cuenta. Si quieres que lo escriba el cliente, abre los **Module Settings** del producto y pon **Customer username** en `Customer types it in the "Line username" or "Reseller username" custom field`. La opción aparece en productos **Line** y **Sub-Reseller** y se oculta en **Credit top-up**; en los productos Sub-Reseller llega con el módulo 1.10.0.
...
2. **Field Name:** `Line username` en un producto Line, o `Reseller username` en uno Sub-Reseller. En un producto Line también sirven `Username` y `Panel username`; en uno Sub-Reseller, `Sub-Reseller username`, `Panel username` y `Username`. Un nombre de campo de WHMCS puede llevar además una etiqueta visible detrás de una barra vertical, como `Line username|Username for the panel`, y desde el módulo 1.9.4 cuentan las dos partes.
...
- Si está libre, la cuenta se crea con ese usuario y la contraseña se genera con tus ajustes de credenciales, igual que siempre (o se toma del campo del cliente cuando **Customer password** está activo, mira "Dejar que el cliente elija su contraseña" más abajo).
...
La comprobación se hace en el carrito y otra vez cuando WHMCS crea el servicio, no solo cuando el cliente hace el pedido, así que un usuario puede estar libre en el carrito y quedar ocupado unos minutos después. Cuando eso pasa, edita el usuario en el servicio y pulsa **Create** otra vez, o pídele al cliente que elija otro. Si dejas el custom field vacío, o dejas la opción en `Generated by the module (default)`, no cambia nada: los usuarios se siguen generando como antes.

**En un producto Sub-Reseller** los dos momentos son los mismos, con una diferencia: la búsqueda de "ya existe" contra el panel solo se hace cuando la entrada del panel usa una clave **Admin**. El carrito entonces responde `The reseller username "X" is already taken. Choose another one.` y al crear el servicio el módulo responde `The reseller username "X" is already taken on this panel. Ask the customer to choose another one.`, y el pedido queda pendiente. Con una clave **Reseller** el módulo no busca el nombre por su cuenta: el carrito lo acepta y es el panel quien rechaza un duplicado al crear la cuenta, así que el pedido queda pendiente con el mensaje del panel. Un valor que no cumpla el formato responde `The username "X" is not valid: use 3 to 32 letters, digits, dashes or underscores.`, y un panel que no responda nunca bloquea el carrito. Los productos Credit top-up ignoran la opción, y en ellos se oculta.

### Dejar que el cliente elija su contraseña

Desde el módulo 1.10.0, en productos **Line** y **Sub-Reseller**. Por defecto el módulo crea la contraseña con tus ajustes del **Password Generator**. Si quieres que la escriba el cliente, pon **Customer password** en `Customer types it in the "Panel password" custom field` y añade un **Custom Field** al producto (Products/Services → tu producto → Custom Fields):

1. **Field Name:** `Panel password` (los nombres `Password`, `Line password` y `Reseller password` también sirven, y cuentan las dos partes de un nombre como `Panel password|Password for the panel`).
2. **Field Type:** `Password` es el tipo recomendado, así WHMCS guarda el valor como secreto en el formulario de pedido y en el servicio.
3. Marca **Show on Order Form** para que el cliente lo vea al hacer el pedido. Marca **Required** solo si quieres obligar a que todos los pedidos lleven contraseña.
4. Guarda.

El módulo acepta de 8 a 32 caracteres, sin espacios y sin ninguno de `% & ? # / \ +`; cualquier otro carácter vale, incluidos `@ $ = : ; ! * ( ) ' ~ , . - _`. Un valor que no cumpla las reglas detiene el formulario de pedido con `The password is not valid: use 8 to 32 characters without spaces and without % & ? # / \ +`, y ese mismo mensaje detiene el paso Create y deja el pedido pendiente. Si dejas el custom field vacío, o dejas la opción en `Generated by the module (default)`, las contraseñas se siguen generando como antes. El módulo nunca muestra la contraseña ni la escribe en los Module Logs.

**Dónde vive la contraseña.** En cuanto se crea la cuenta, el módulo vacía ese custom field en el servicio de WHMCS, así que la contraseña queda solo en el campo Password del servicio, como una generada, y ahí es donde la ve el cliente. Hasta que el servicio existe (un pedido pendiente, un pago sin confirmar) el valor sigue en el custom field, y si tu plantilla de email **Order Confirmation** de WHMCS muestra los custom fields, la contraseña puede viajar en ese email: usa un campo de tipo **Password** y revisa esa plantilla si activas la opción.

**Qué pasa cuando WHMCS crea el servicio:**

Cuando un cliente compra (y el pago se confirma), WHMCS hace esto solo:

1. Genera un **usuario** y una **contraseña** (o usa los que escribió el cliente al hacer el pedido cuando las opciones **Customer username** y **Customer password** están activas).
...
Con una llave **Admin** el monto es un ajuste: suma o quita créditos en esa cuenta sin tocar a nadie más. Con una llave **Reseller** (módulo 1.12.0 o posterior) la pantalla muestra los sub-resellers propios de ese reseller, y los de ellos, y la columna **Transfer** (módulo 1.13.0 o posterior) mueve créditos: un monto positivo sale del saldo propio del reseller, y uno negativo le quita créditos a un sub-reseller directo y los devuelve al saldo del reseller (solo si el panel lo permite). El reseller nunca puede sumarse créditos a su propia cuenta.

Una llave Reseller necesita el permiso **See sub-resellers** (`subresellers:read`) para listar y **Transfer credits to sub-resellers** (`subresellers:credits`) para transferir; además, el administrador del panel debe tener habilitados los sub-resellers para el grupo del reseller. Las llaves no se pueden editar: crea una llave nueva en el panel marcando esos permisos y pega el token nuevo en el panel configurado. Los mensajes `this panel key is not allowed to list sub-resellers` y `This panel key is not allowed to transfer credits` indican que a la llave le falta uno de ellos.
...
Desde el módulo 1.9.1 el carrito también valida el campo `Reseller username`, al añadir el producto y otra vez en la página de confirmación del pedido, para que el cliente vea el error antes de pagar. Con el **Top-up scope** en `Any reseller on the panel`, un usuario que no exista en ninguna cuenta del panel detiene el formulario de pedido con `The reseller username "X" was not found. Check the spelling and try again.`, y uno que pertenezca a un administrador del panel lo detiene con `The reseller username "X" cannot receive a top-up.` Con el **Top-up scope** en `Only this client's linked Sub-Reseller accounts`, un cliente que ha iniciado sesión y escribe un usuario que no corresponde a ninguna de sus cuentas Sub-Reseller vinculadas ve `The reseller username "X" does not match any of your Sub-Reseller accounts.`; a un visitante que no ha iniciado sesión no se le comprueba en el carrito y su pedido se rechaza al crear el servicio, igual que antes. El carrito solo comprueba cuando el campo tiene valor y necesita una clave **Admin** en la entrada del panel; si el panel no responde, no bloquea el pedido y la cuenta se resuelve otra vez al crear el servicio.
...
**Sub-Resellers** — la lista de sub-resellers y sus créditos, con el ajuste de créditos (llave Admin). Con una llave Reseller muestra solo los sub-resellers propios de ese reseller y transfiere créditos desde su saldo (ver sección 10).
...
| **The reseller username "X" is already taken on this panel. Ask the customer to choose another one.** | En un producto Sub-Reseller con la opción **Customer username**, el usuario que escribió el cliente pertenece a una cuenta que ya existe en el panel. El módulo lo buscó porque la entrada del panel usa una clave **Admin**. No se creó nada y el pedido queda pendiente. | Pídele otro usuario al cliente, corrígelo en el campo `Reseller username` del servicio de WHMCS y vuelve a pulsar **Create**. Con una clave **Reseller** el duplicado lo rechaza el propio panel y el pedido lleva su mensaje. |
| **Set the Sub-Reseller Member Group ID in this product's Module Settings: with an Admin panel key the panel needs the numeric id of the member group the new account belongs to.** | Se hizo un pedido de un producto Sub-Reseller y la entrada del panel usa una clave **Admin**, pero al producto le falta el **Sub-Reseller Member Group ID**. El módulo se detiene antes de crear nada, así que no se creó nada y el pedido queda pendiente. | Abre los **Module Settings** del producto y rellena **Sub-Reseller Member Group ID** con el id numérico del grupo de miembros al que deben pertenecer las nuevas cuentas: lo tienes en la página **Member Groups** del panel. Guarda el producto y vuelve a pulsar **Create**. Las claves de Reseller no lo necesitan: heredan el grupo desde su propia configuración de sub-reseller. |
| **The password is not valid: use 8 to 32 characters without spaces and without % & ? # / \ +** | En un producto con la opción **Customer password**, el valor del campo `Panel password` incumple alguna regla. No se creó nada y el pedido queda pendiente. | Pídele al cliente una contraseña que cumpla las reglas, corrígela en el servicio de WHMCS y vuelve a pulsar **Create**, o deja el campo vacío para que el módulo la genere. |
...
Sí, en productos Line y Sub-Reseller. Pon **Customer username** del producto en `Customer types it in the "Line username" or "Reseller username" custom field` y añade a ese producto un custom field: `Line username` en un producto Line, o `Reseller username` en uno Sub-Reseller (Text Box, marcado **Show on Order Form**). El cliente escribe el usuario al hacer el pedido y el módulo lo usa si es válido y está libre en el panel. El módulo también comprueba el campo en el carrito, al añadir el producto y otra vez al confirmar el pedido, así que un usuario ya tomado normalmente se rechaza antes de que el cliente pague; cuando el pedido sí llega con un usuario tomado, queda pendiente con un mensaje claro y le dices al cliente que elija otro. En un producto Sub-Reseller la búsqueda en el panel necesita una clave **Admin** en la entrada del panel; con una clave **Reseller** el carrito acepta el nombre y es el panel quien rechaza un duplicado al crear la cuenta. El módulo nunca vincula una línea que ya existe a partir de un pedido del cliente, así nadie puede quedarse con la línea de otro. Mira "Dejar que el cliente elija su usuario" en la sección 6.

**¿Mis clientes pueden elegir también su propia contraseña?**
Sí, en productos Line y Sub-Reseller, desde el módulo 1.10.0. Pon **Customer password** del producto en `Customer types it in the "Panel password" custom field` y añade a ese producto un custom field llamado `Panel password`: el tipo **Password** es el recomendado, marca **Show on Order Form** y marca **Required** solo si quieres obligar. El módulo acepta de 8 a 32 caracteres, sin espacios y sin `% & ? # / \ +`, y rechaza lo demás en el carrito y al crear el servicio. En cuanto la cuenta existe el módulo vacía ese campo en el servicio y la contraseña queda solo en el campo Password del servicio, pero hasta entonces el valor está en el custom field, así que deja el tipo **Password** y revisa que tu email Order Confirmation de WHMCS no imprima los custom fields. Mira "Dejar que el cliente elija su contraseña" en la sección 6.
