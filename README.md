...
- Per-product **Customer username** option (Line and Sub-Reseller
  products): the username of the line or of the sub-reseller account can
  come from a WHMCS custom field the customer fills in at checkout,
  instead of being generated by the module. The module validates the
  value, checks the panel and creates the account with it; a username
  that is already taken fails with a clear message and the order stays
  pending. The module never links an existing line from a customer order
  (Sub-Reseller products: module 1.10.0 and later).
- Per-product **Customer password** option (Line and Sub-Reseller
  products, module 1.10.0 and later): the password can come from a WHMCS
  custom field the customer fills in at checkout, 8 to 32 characters with
  no spaces and none of `% & ? # / \ +`. The module empties that field on
  the service once the account exists, so the password lives only in the
  service's Password field, like a generated one.
...
- Username and password generated at creation, or taken from the
  product's `Reseller username` and `Panel password` custom fields when
  the **Customer username** and **Customer password** options are on,
  with the panel checked for a duplicate username on an admin key
  (module 1.10.0 and later).
...
4. Fill the config options: Panel, Package Type, Package, Bouquets,
   ...
   and Sub-Reseller products: whether the module generates the username
   or takes the one the customer typed in the `Line username` or
   `Reseller username` custom field, see "Customer-chosen usernames"
   below), Customer password (Line and Sub-Reseller products: whether the
   module generates the password or takes the one the customer typed in
...
**Line** and **Sub-Reseller** products (Sub-Reseller products: module
1.10.0 and later), and it is hidden on Credit top-up. It has two values:
...
- **Customer types it in the "Line username" or "Reseller username"
  custom field:** the username comes from a WHMCS custom field on the
  product, `Line username` on a Line product and `Reseller username` on
  a Sub-Reseller one.
...
too; a Sub-Reseller product also accepts `Sub-Reseller username`. A WHMCS
field name may carry a visible label after a pipe
...
panel and, when it is free, creates the line with that username. The
password comes from the addon's Credentials settings (the Password
Generator), or from the customer's own field when **Customer password**
is on (see "Customer-chosen passwords" below). Two failures are possible:
...
**Sub-Reseller products** (module 1.10.0 and later) follow the same two
moments, with the panel lookup made against an account instead of a line.
That lookup runs only when the panel entry uses an **Admin** key: the
cart then answers `The reseller username "X" is already taken. Choose
another one.` and the create step answers `The reseller username "X" is
already taken on this panel. Ask the customer to choose another one.`,
leaving the order pending. With a **Reseller** key the module does not
look the name up itself: the cart accepts it and the panel refuses a
duplicate when the account is created, so the order stays pending with
the panel's own message. An invalid value answers `The username "X" is
not valid: use 3 to 32 letters, digits, dashes or underscores.`, and a
panel that does not answer never blocks the cart, because the name is
checked again at create time. Credit top-up products ignore the option,
and it is hidden on them.

### Customer-chosen passwords

Module 1.10.0 and later, on Line and Sub-Reseller products. The
**Customer password** config option appears in Module Settings with both
Account Types and is hidden on Credit top-up. It has two values:

- **Generated by the module (default):** the password comes from the
  Password Generator of General Settings, as it always has.
- **Customer types it in the "Panel password" custom field:** the
  password comes from a WHMCS custom field on the product.

With the second value, add a custom field to the product named
`Panel password` (also accepted: `Password`, `Line password`,
`Reseller password`; both halves of a name like
`Panel password|Password for the panel` count), type **Password**, which
is the recommended WHMCS type so the value is treated as a secret, with
**Show on Order Form** ticked so the customer types it at checkout. Tick
**Required** only if every order has to carry a password.

The module accepts 8 to 32 characters with no spaces and none of
`% & ? # / \ +`. Every other character is fine, including
`@ $ = : ; ! * ( ) ' ~ , . - _`. A value that does not fit stops the
cart with `The password is not valid: use 8 to 32 characters without
spaces and without % & ? # / \ +`, and the same message stops the create
step and leaves the WHMCS order pending. With the option off, or with an
empty custom field, passwords are generated from General Settings exactly
as before. The module never displays the password and never writes it to
the module log.

**Where the chosen password lives.** As soon as the account exists the
module empties the `Panel password` field on the WHMCS service, so the
password stays only in the service's Password field, like a generated
one, and that is where the customer sees it in the client area. Until
then (a pending order, a payment not confirmed) the value is still in the
custom field, and if the WHMCS **Order Confirmation** email template
renders custom fields the password can travel in that email: use the
**Password** field type and review that template if you turn the option
on for a product.
...
- **Line** (default): each service becomes one IPTV line on the panel.
  Package and bouquets come from the panel, and the username and the
  password are generated by the module unless the product's **Customer
  username** and **Customer password** options take them from the
  `Line username` and `Panel password` custom fields; Suspend/Unsuspend
  toggle the line status unless the product's **Suspend action** says to
  leave the line untouched; Renew moves the panel expiry, re-applies the
  product's connection count when the panel reset it to the package
  value, and syncs WHMCS's next due date; Terminate deletes the line on
  the panel. A line that already existed on the panel is adopted with
  **Link existing line** instead of being created again.
- **Sub-Reseller:** each service becomes a sub-reseller account on the
  ...
  password. From module 1.10.0 the **Customer username** and **Customer
  password** options can take the credentials from the `Reseller
  username` and `Panel password` custom fields instead, with the panel
  checked for a duplicate username when the panel entry uses an admin
  key. Suspend, Unsuspend and Terminate require a manual action on
